...
if h_in > h_LS % check if fluid is biphase
    ...
    % compute alpha convection (aumentato artificialmente)
    ...
    if L_bi > data.L_condenser
        T_6 = T_sat;
        %display('Exit condenser biphase')
        return
    end
...
% Compute Reynolds number to verify laminar flow
%
% NORMALLY Re is greater than 10.000. Laminar flow condition in a pipe is
% Re < 2400

% So we have to use a different correlation (Dittus-Boelter)
